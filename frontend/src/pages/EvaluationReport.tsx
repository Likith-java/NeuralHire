...
const QuestionRow: React.FC<{ item: any; isRecruiter: boolean }> = ({ item, isRecruiter }) => {
  ...
  return (
    <>
      ...
      >
        ...
        <td className="py-4 px-4 text-xs text-text-muted italic opacity-70 group-hover:opacity-100 transition-opacity">{isRecruiter ? item.aria_feedback : 'Analysis complete.'}</td>
          ...
          >
            <td colSpan={7} className="p-6">
              <div className="space-y-4">
                ...
                {isRecruiter && item.aria_feedback && (
      ...
      >
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {report.answers?.map((item: any, idx: number) => (
                  <QuestionRow key={idx} item={item} isRecruiter={isRecruiter} />
                ))}
              </tbody>
            </table>
          </div>
        ...
        <motion.footer variants={itemVariants} className="pt-10 pb-20 border-t border-border flex flex-col items-center gap-4 text-center">
          ...
          <p className="text-[10px] font-mono text-text-muted uppercase tracking-[0.2em] font-bold">
            Generated by NeuralHire OS v2.0 · 5 ML Models Active
          </p>
