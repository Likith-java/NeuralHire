...
class VerifierEngine:
    def __init__(self):
        ...
        self.body_pose = BodyPoseEstimator()
    ...
    def process_frame(self, frame):
        ...
        posture = self.body_pose.get_posture(frame)
        ...
        if posture is not None:
            if posture > 40:  # threshold
                if not self.prev_posture_flag:
                    self.events["bad_posture"] += 1
                    self.prev_posture_flag = True
            else:
                self.prev_posture_flag = False
        ...
        score -= self.events["bad_posture"] * 2

        score = max(0, score)

        return score, self.events, yaw, face_count, gaze, posture
